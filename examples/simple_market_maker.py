...
class MarketMakerBot:
    def __init__(self):
        ...
        self.cloid_status = (
            {}
        )  # Tracks 'active', 'filled', 'cancelled', 'partially_filled'
    ...
    def on_order_created(self, payload: OrderCreatedPayload):
        ...
        if payload.cloid in self.cloid_to_order:
            ...
            if payload.is_canceled:
                ...
                if payload.cloid in self.active_cloids:
                    self.active_cloids.remove(payload.cloid)
        ...
        if order_id in self.order_id_to_cloid:
            ...
            if cloid in self.cloid_to_order:
                ...
                if payload.updated_size == 0:
                    ...
                    print(
                        f"Order {cloid} partially filled. Remaining size: {payload.updated_size}"
                    )

                print(
                    f"Updated order details for {cloid} after trade: {self.cloid_to_order[cloid]}"
                )
            else:
        ...
        for order_id in payload.order_ids:
            if order_id in self.order_id_to_cloid:
                ...
                if cloid in self.cloid_to_order:
                    ...
                    print(
                        f"Updated order details for {cloid}: {self.cloid_to_order[cloid]}"
                    )
                else:
                    print(
                        f"Received cancellation for cloid {cloid} not in local cache."
                    )
            else:
    ...
    async def place_mm_orders(self):
        ...
        orders = []
        print(
            f"Placing {NUM_ORDERS_PER_SIDE} buy and {NUM_ORDERS_PER_SIDE} sell orders..."
        )
        ...
        for i in range(NUM_ORDERS_PER_SIDE):
            ...
            print(
                f"Prepared Buy: cloid={buy_cloid}, price={buy_price}, size={ORDER_SIZE}"
            )
            print(
                f"Prepared Sell: cloid={sell_cloid}, price={sell_price}, size={ORDER_SIZE}"
            )
        ...
        try:
            ...
            for order in orders:
                if (
                    order.cloid in self.cloid_status
                    and self.cloid_status[order.cloid] == "pending_creation"
                ):
...
if __name__ == "__main__":
    try:
        ...
        print(
            "KeyboardInterrupt caught in __main__. Shutdown initiated by signal handler."
        )
    except Exception as e:
